...
async function generatePrimaryAndUniqueConstraint(client, schemaName) {
  ...
  `;
    ...
    const outOfLineConstraintList = rows.filter((constraint) => constraint.columnCount > 1);
    const compositeTableConstraintMap = outOfLineConstraintList.reduce((acc, row) => {
...
function extractColumnsFromCheckConstraint(checkClause) {
    return [...checkClause.matchAll(/`(?:``|[^`])*`/g)].map((col) => col.toString().slice(1, -1));
}
async function generateCheckConstraints(client, schemaName) {
    const query = `
    select
  ...
  `;
    ...
    rows.forEach((row) => {
        const { constraintName, tableName, checkClause, } = row;
        const key = tableName;
        ...
        if (cols.length === 1) {
            ...
            if (!tableConstraints[key][col])
                tableConstraints[key][col] = { checks: [] };
...
async function fetchSchemaJson(connection) {
    ...
    client.end();
    const [{ tableList, fieldMap }, enumList, rawIndexMap, { primaryAndUniqueConstraintMap, compositeConstraintMap }, foreignKeyList, { checks: tableCheckConstraintMap, tableConstraints: columnCheckConstraintMap },] = result;
    ...
    const tableConstraints = { ...primaryAndUniqueConstraintMap };
    Object.keys(columnCheckConstraintMap).forEach((tableName) => {
        if (!tableConstraints[tableName])
            tableConstraints[tableName] = {};
        Object.keys(columnCheckConstraintMap[tableName]).forEach((colName) => {
            if (!tableConstraints[tableName][colName])
                tableConstraints[tableName][colName] = { checks: [] };
