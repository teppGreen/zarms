...
const generateIndexesAndConstraints = async (client, schemas) => {
    const indexListSql = `
    WITH user_tables AS (
      SELECT
  ...
  `;
    ...
    const { outOfLineIndexes, inlineIndexes } = indexListResult.rows.reduce((acc, row) => {
        ...
        if (index_columns === 'null' || index_columns.trim() === '')
            return acc;
    ...
    const indexes = outOfLineIndexes.reduce((acc, indexRow) => {
        const { table_schema, table_name, index_name, index_type, index_columns, index_expressions, is_primary, is_unique, } = indexRow;
        const indexColumns = index_columns.split(',').map((column) => {
            return {
                type: 'column',
                value: column.trim(),
            };
        ...
        const indexExpressions = index_expressions
            ? index_expressions.split(',').map((expression) => {
                return {
                    type: 'expression',
                    value: expression,
                };
    ...
    inlineIndexes.forEach((row) => {
        const { table_schema, table_name, index_columns, constraint_type, } = row;
        const key = `${table_schema}.${table_name}`;
        if (!tableConstraints[key])
            tableConstraints[key] = {};
        const columnNames = index_columns.split(',').map((column) => column.trim());
        columnNames.forEach((columnName) => {
            if (!tableConstraints[key][columnName])
                tableConstraints[key][columnName] = { checks: [] };
    ...
    ${(0, parseSchema_1.buildSchemaQuery)('n.nspname', schemas)}
    GROUP BY
      con.conname,
      n.nspname,
      c.relname,
  ...
  `;
    ...
    checkListResult.rows.forEach((row) => {
        const { table_schema, table_name, check_name, check_definition, column_name, is_column_constraint, } = row;
        ...
        if (is_column_constraint) {
            if (!tableConstraints[key])
                tableConstraints[key] = {};
            if (!tableConstraints[key][column_name])
                tableConstraints[key][column_name] = { checks: [] };
