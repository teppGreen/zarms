...
const generateTablesFields = async (client, schemas) => {
    const fields = {};
...
const generateIndexesAndConstraints = async (client, schemas) => {
  ...
  `;
    ...
    const indexes = outOfLineIndexes.reduce((acc, indexRow) => {
        const { table_schema, table_name, index_name, index_type, columns, expressions, is_unique, is_primary, } = indexRow;
    ...
    WHERE 1 = 1
      ${(0, parseSchema_1.buildSchemaQuery)('OBJECT_SCHEMA_NAME(cc.parent_object_id)', schemas)}
  `;
    const checkListResult = await client.query(checkListSql);
    const checks = {};
    checkListResult.recordset.forEach((row) => {
        const { table_schema, table_name, column_name, check_name, check_expression, } = row;
        const key = `${table_schema}.${table_name}`;
        if (column_name) {
            if (!tableConstraints[key])
                tableConstraints[key] = {};
